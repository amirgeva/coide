...
class Qutepart(QPlainTextEdit):
    ...
    **Public methods**
    ...
    def colorLine(self,line,color):
        '''
        line is 1 based
        color is a string like '#ff00ff'
        '''
        if (line<=0):
            self.setExtraSelections([])
        else:
            self._userExtraSelectionFormat.setBackground(QBrush(QColor(color)))
            start=self.mapToAbsPosition(line-1,0)
            end=self.mapToAbsPosition(line,0)
            self.setExtraSelections([(start,end-start)])
